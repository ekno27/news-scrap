...
app.get("/articles/:id",function(req,res){
    db.MusicArticle.findOne({_id:req.params.id})
    .populate("note")
...
//add the note to its corresponding article
app.post("/articles/:id",function(req,res){
    console.log(req.body);
    db.Note.create(req.body)
    .then(function(dbNote){
        return db.MusicArticle.findOneAndUpdate({_id:req.params.id},{note: dbNote._id }, {new: true});
    }).then(function(dbArticle){
        res.json(dbArticle);
    }).catch(function(err){
        console.log(err);
